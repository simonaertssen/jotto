...
def build_graph(wordmap: dict[int: str]) -> dict[int: set[int]]:
    """
    Build a directed acyclic graph from the different bitwords that
    exist in the dataset. This can be done by visiting all other words
    in the vector with encoded words. Hence, we only map a directed graph.
    """
